...
class IncrementalGaussianProcess:
    def __init__(self, kernel_lambda):
        ...
        self._x = None
        self._y = None
        self._first_fitted_variance = False
        self._first_fitted_mean = False
        self._l_matrix = None
    ...
    def update_variance(self, train_x):
        if not self._first_fitted_variance:
            self.update_variance_first(self, train_x)
    ...
    def update_variance_first(self, train_x):
        train_x = np.array(train_x)
        ...
        self._first_fitted_variance = True

        
    def update_mean_first(self, train_y):
        ...
        self._first_fitted_mean = True
        
        
    def update_mean(self, train_y):
        if not self._first_fitted_mean:
            self.update_mean_first(self, train_y)
        train_y = np.array(train_y)
    ...
    def incremental_fit(self, train_x, train_y):
        if not (self._first_fitted_variance and self._first_fitted_mean):
            raise ValueError("The first_fit function needs to be called first.")

        self.update_variance(train_x)
        self.update_mean(train_y)

        return self

    @property
    def first_fitted(self):
        return (self._first_fitted_variance and self._first_fitted_mean)

    def first_fit(self, train_x, train_y):
        self.update_variance_first(train_x)
        self.update_mean_first(train_y)
        return self

    def predict(self, train_x):
        #print("Bayesian X:",self._x)
        #print("Bayesian Y:",self._y)
