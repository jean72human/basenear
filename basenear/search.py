...
class BayesianSearcher:
    ...
    def search(self, train_data, test_data):
        ...
        train_results = train((graph, train_data, test_data, self.trainer_args, None, self.verbose, self.device))
...
class GPUCBPESearcher:
    ...
    def __init__(self, n_classes, input_shape, path, verbose, n_parallel,
                 trainer_args=None,
                 default_model_len=Constant.MODEL_LEN,
                 default_model_width=Constant.MODEL_WIDTH,
                 beta=Constant.BETA,
        ...
        if t_min is None:
            t_min = Constant.T_MIN
        self.t_min = t_min
        self.devices = ['cuda:0' for i in range(self.n_parallel)]
    ...
    def add_model(self, accuracy, loss, graph, model_id):
        ...
        #escriptor = graph.extract_descriptor()
        self.y_queue.append(accuracy)
        self.gpr.update_mean(self.y_queue)
    ...
    def train_from_queue(self, k):
        ...
        ##multiprocessing.set_start_method('spawn', force=True)
        ##pool = multiprocessing.Pool(0)
        train_results = train((graph, train_data, test_data, self.trainer_args, None, self.verbose, self.device[k]))
    ...
    def search(self, train_data, test_data):
        ...
        if not self.training_queue:
            ...
            self.x_queue.append(descriptor)
            self.descriptors.append(new_graph.extract_descriptor())
            self.gpr.update_variance(self.x_queue)
            
            for i in range(self.n_parallel - 1):
                new_graph, new_father_id = self.maximize_std()
                new_model_id = self.model_count
                self.model_count += 1
                self.training_queue.append((new_graph, new_father_id, new_model_id))
                descriptor = new_graph.extract_descriptor()
        ...
        processes = [Process(target=self.train_from_queue, args=(k,)) for k in range(self.n_parallel)]
        # Run processes
        for p in processes:
            p.start()
        ...
        for p in processes:
            p.join()
        #elf.gpr.fit(self.x_queue, self.y_queue)
        self.x_queue = []
        self.y_queue = []
    ...
    def maximize_std(self):
        ...
        while not pq.empty() and t > t_min:
            elem = pq.get()
            temp_exp = min((elem.accuracy - max_std) / t, 709.0)
            ap = math.exp(temp_exp)
            if ap > random.uniform(0, 1):
                graphs = transform(elem.graph)
                ...
                for temp_graph in graphs:
                    ...
                    temp_std_value = self.std(temp_graph)
                    pq.put(Elem(temp_std_value, elem.father_id, temp_graph))
                    descriptors.append(temp_graph.extract_descriptor())
                    if temp_std_value > max_std:
                        max_std = temp_std_value
                        father_id = elem.father_id
                        target_graph = temp_graph
            t *= alpha
    ...
    def std(self, graph):
        std = self.gpr.get_variance(np.array([graph.extract_descriptor()]))
        return std
...
def train(args):
    graph, train_data, test_data, trainer_args, path, verbose, device = args
